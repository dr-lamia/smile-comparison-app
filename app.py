...
st.write(
    "Upload the AI-generated smile image and the conventional smile design image from the same case."
)
...
def crop_region(img, x1, y1, x2, y2):
    ...
    x2_px = max(1, min(x2_px, w))
    y1_px = max(0, min(y1_px, h - 1))
    y2_px = max(1, min(y2_px, h))
...
def save_bgr_image_temp(img_bgr, filename):
...
def generate_pdf_report(
    ...
    pdf_path = os.path.join(temp_dir, f"{safe_case_id}_smile_comparison_report.pdf")
    ...
    story.append(
        Paragraph(
            "AI-Generated Smile vs Conventional Smile Design Comparison Report",
            styles["Title"],
        )
    )
    ...
    story.append(Paragraph("<b>Summary of Quantitative Metrics</b>", styles["Heading2"]))
    ...
    data = [
        ...
        [
            "Full-face SSIM",
            f"{full_ssim:.4f}",
            "Global facial similarity and identity preservation",
        ],
        [
            "Full-face MSE",
            f"{full_mse:.2f}",
            "Pixel-level full-face difference",
        ],
        [
            "Smile-region SSIM",
            f"{mouth_ssim:.4f}",
            "Dentolabial design similarity",
        ],
        [
            "Smile-region MSE",
            f"{mouth_mse:.2f}",
            "Pixel-level smile-region difference",
        ],
    ]

    table = Table(data, colWidths=[1.7 * inch, 1.2 * inch, 3.4 * inch])
    table.setStyle(
        TableStyle(
            [
    ...
    interpretation = (
        "Full-face SSIM and MSE were used to evaluate global facial similarity "
        "and identity preservation. Smile-region SSIM and MSE were used to evaluate "
        "dentolabial design similarity. Paired images were aligned using "
        "ECC-based affine registration before comparison. A higher SSIM indicates "
        "greater structural similarity, whereas a lower MSE indicates lower "
        "pixel-level difference."
    )
    story.append(Paragraph(interpretation, styles["Normal"]))
    story.append(Spacer(1, 14))

    full_ai_path = save_bgr_image_temp(img_ai, f"{safe_case_id}_ai_full.png")
    full_conv_path = save_bgr_image_temp(
        img_conv_aligned, f"{safe_case_id}_conv_full.png"
    )
    full_overlay_path = save_bgr_image_temp(
        full_overlay, f"{safe_case_id}_full_overlay.png"
    )

    mouth_ai_path = save_bgr_image_temp(mouth_ai, f"{safe_case_id}_ai_mouth.png")
    mouth_conv_path = save_bgr_image_temp(
        mouth_conv_aligned, f"{safe_case_id}_conv_mouth.png"
    )
    mouth_overlay_path = save_bgr_image_temp(
        mouth_overlay, f"{safe_case_id}_mouth_overlay.png"
    )

    story.append(Paragraph("<b>Full-Face Comparison</b>", styles["Heading2"]))

    full_img_table = Table(
        [
            [
                RLImage(full_ai_path, width=1.75 * inch, height=2.25 * inch),
                RLImage(full_conv_path, width=1.75 * inch, height=2.25 * inch),
                RLImage(full_overlay_path, width=1.75 * inch, height=2.25 * inch),
            ],
            [
                "AI-generated smile",
                "Conventional design aligned",
                "Full-face overlay",
            ],
        ]
    )

    full_img_table.setStyle(
        TableStyle(
            [
    ...
    story.append(full_img_table)
    story.append(Spacer(1, 16))

    story.append(Paragraph("<b>Smile-Region Comparison</b>", styles["Heading2"]))

    mouth_img_table = Table(
        [
            [
                RLImage(mouth_ai_path, width=1.75 * inch, height=1.05 * inch),
                RLImage(mouth_conv_path, width=1.75 * inch, height=1.05 * inch),
                RLImage(mouth_overlay_path, width=1.75 * inch, height=1.05 * inch),
            ],
            [
                "AI smile region",
                "Conventional region aligned",
                "Smile-region overlay",
            ],
        ]
    )

    mouth_img_table.setStyle(
        TableStyle(
            [
                ("ALIGN", (0, 0), (-1, -1), "CENTER"),
                ("FONTSIZE", (0, 1), (-1, 1), 8),
    ...
    story.append(mouth_img_table)
    story.append(Spacer(1, 12))

    story.append(Paragraph("<b>Methodological Note</b>", styles["Heading2"]))
    note = (
        "The report is intended for research and documentation purposes. "
        "Similarity metrics should be interpreted together with expert esthetic "
        "assessment and morphologic measurements such as midline deviation, "
        "smile width, incisal curve depth, and symmetry."
    )
...
with col1:
    file_ai = st.file_uploader(
        "Upload AI-generated smile image", type=["jpg", "jpeg", "png"]
    )

with col2:
    file_conv = st.file_uploader(
        "Upload conventional smile design image", type=["jpg", "jpeg", "png"]
    )
...
if st.button("Compare AI vs Conventional Design"):
    if file_ai is None or file_conv is None:
        ...
        with c1:
            st.image(
                to_rgb(img_ai),
                caption="AI-generated smile",
                use_container_width=True,
            )

        with c2:
            st.image(
                to_rgb(img_conv_aligned),
                caption="Conventional smile design (aligned)",
                use_container_width=True,
            )
        ...
        with c4:
            st.image(
                to_rgb(mouth_ai),
                caption="AI smile region",
                use_container_width=True,
            )

        with c5:
            st.image(
                to_rgb(mouth_conv_aligned),
                caption="Conventional smile region (aligned)",
                use_container_width=True,
            )

        with c6:
            st.image(
                to_rgb(mouth_overlay),
                caption="Smile-region overlay",
                use_container_width=True,
            )
        ...
        case_id = st.text_input("Case ID for report", value="case_001")

        pdf_path = generate_pdf_report(
            case_id=case_id,
            full_ssim=full_ssim,
            full_mse=full_mse,
            mouth_ssim=mouth_ssim,
            mouth_mse=mouth_mse,
            img_ai=img_ai,
            img_conv_aligned=img_conv_aligned,
            full_overlay=full_overlay,
            mouth_ai=mouth_ai,
            mouth_conv_aligned=mouth_conv_aligned,
        ...
        with open(pdf_path, "rb") as pdf_file:
            st.download_button(
                ...
                data=pdf_file,
                file_name=f"{case_id}_smile_comparison_report.pdf",
